...
def plot_sigmoids(results: pd.DataFrame, out_dir: Path) -> None:
    ...
    for ax, row in zip(axes, results.itertuples(index=False)):
        x_max = max(1.0, float(row.test_a_p99))
        x = np.linspace(0.0, x_max, 300)
        y = sigmoid_weight(x, row.a_scale, row.sigmoid_slope, row.sigmoid_bias)
        ax.plot(x, y, color="#d55e00", linewidth=2.5)
        ax.axhline(0.5, color="gray", linestyle="--", linewidth=1)
        ax.set_title(f"{row.dataset}\nAUC={row.sigmoid_auc:.3f}")
        ax.set_xlabel("a = prior attempts on target KC(s)")
    ...
    fig.suptitle("Sigmoid Ensemble by KC Attempt Count", fontsize=16)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    fig.savefig(out_dir / "sigmoid_a_curves.png", dpi=200)
    fig.savefig(out_dir / "sigmoid_a_curves.pdf")
    plt.close(fig)


def main() -> None:
    parser = argparse.ArgumentParser(description="Train an a-dependent sigmoid StaticKT/PFA ensemble.")
    parser.add_argument("--data-root", type=Path, default=Path("data"))
    parser.add_argument("--output-dir", type=Path, default=Path("results/sigmoid_a_ensemble"))
    ...
    for dataset in datasets:
        ...
        if not train_path.exists() or not test_path.exists() or not x_path.exists() or not q_path.exists():
        ...
        a_val = kc_attempt_counts(val_df, q_path, preserve_original_order=False)
        a_scale = max(1.0, float(np.percentile(a_val, 95)))
        ...
        a_test = kc_attempt_counts(test_df, q_path, preserve_original_order=True)
        pred = sigmoid_blend(prior_test, pfa_test, a_test, a_scale, slope, bias)
        metrics = compute_metrics(y_test, pred)
        a_p99 = float(np.percentile(a_test, 99))

        out_df = test_df.copy()
        out_df["a_kc_prior_attempts"] = a_test
        out_df["PFA_WEIGHT_SIGMOID_A"] = sigmoid_weight(a_test, a_scale, slope, bias)
        out_df["SIGMOID_A_ENSEMBLE"] = pred
        out_df.to_csv(args.output_dir / f"{dataset}_sigmoid_a_predictions.csv", index=False)
        ...
        rows.append(
            {
                ...
                "sigmoid_slope": slope,
                "sigmoid_bias": bias,
                "pfa_weight_at_a0": float(sigmoid_weight(np.array([0.0]), a_scale, slope, bias)[0]),
                "pfa_weight_at_ap99": float(sigmoid_weight(np.array([a_p99]), a_scale, slope, bias)[0]),
                **metrics,
            }
        )
        print(f"Sigmoid-a AUC={metrics['auc']:.4f}")
